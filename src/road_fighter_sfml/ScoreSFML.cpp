...
void ScoreSFML::display()
{
        if (endVisual) {
                ...
                if (highScore) {
                        ...
                        highScoreText.setPosition(static_cast<float>(x) - highScoreText.getGlobalBounds().width / 2,
                                                  static_cast<float>(y) - 140);
                ...
                for (const unsigned int& s : scores) {
                        ...
                        hallOfFame.setPosition(static_cast<float>(x) - hallOfFame.getGlobalBounds().width / 2,
                                               static_cast<float>(y) - 50 + 40 * counter);
