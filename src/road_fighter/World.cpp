...
void World::addObserver(const shared_ptr<Score>& observer) { observers.emplace_back(observer); }

void World::addEntity(const shared_ptr<road_fighter::Entity>& entity) { entities.emplace_back(entity); }
...
void World::setPlayer(unique_ptr<road_fighter::PlayerCar> entity) { player = move(entity); }
                ...
                for (const shared_ptr<Entity>& e : entities) {
                        if (areColliding(*entity, *e) && tryCounter < 4) {
                                if (tryCounter < 4) {
                                        ++tryCounter;
                                        entity->updatePos(
                                            Random::getInstance().getRandom(leftBound, rightBound - entity->getWidth()),
                                            entity->getYPos());
                                        colliding = true;
                                        break;
                                } else {
...
void World::spawnBullet(const shared_ptr<Bullet>& entity)
{
        ...
        if (player->canShoot()) {
                entity->updatePos(player->getXPos() + player->getWidth() / 2 - entity->getWidth() / 2,
                                  player->getYPos() - entity->getHeight());
...
void World::addRacingCar(const shared_ptr<RacingCar>& racingcar)
{
        ...
        while (colliding) {
                colliding = false;
                racingcar->updatePos(Random::getInstance().getRandom(leftBound, rightBound - racingcar->getWidth()),
                                     Random::getInstance().getRandom(-3, 3));
...
void World::setLength(const double& l) { length = l; }

double World::getLength() const { return length; }
...
bool World::isGameEnd() const { return gameEnd; }
